...
const Budgets: React.FC = () => {
  ...
  const [filterStatus, setFilterStatus] = useState<'all' | 'healthy' | 'warning' | 'exceeded'>(
    'all'
  );
  const [sortBy, setSortBy] = useState<'name' | 'amount' | 'percentUsed' | 'remaining'>(
    'percentUsed'
  );
  ...
  const getTagName = useCallback((tagId: string) => tagMap.get(tagId)?.name || 'Unknown', [tagMap]);
  ...
  const handleCreateFromTemplate = useCallback(
    (template: (typeof budgetTemplates)[0]) => {
      const { startDate, endDate } = getMonthDateRange();
      setFormData({
        name: template.name,
        categoryIds: [],
  ...
  const handleAmountKeyPress = useCallback(
    (e: React.KeyboardEvent, budgetId: string) => {
      if (e.key === 'Enter') {
  ...
  return (
    <Box>
                ...
                <Grid size={{ sm: 3, xs: 6 }}>
                  <Box>
                    ...
                    <Typography variant="caption" sx={{ opacity: 0.8 }}>
                      {budgetSummary.totalCount}{' '}
                      {budgetSummary.totalCount === 1 ? 'budget' : 'budgets'}
                    </Typography>
                  </Box>
                </Grid>
        ...
        >
          <CardContent sx={{ textAlign: 'center', py: 6 }}>
            ...
            <Typography variant="body1" color="text.secondary" mb={4} maxWidth="500px" mx="auto">
              Take control of your finances by setting spending limits. Choose from templates or
              create custom budgets to track categories, tags, and accounts.
            </Typography>
                      ...
                      <Box flex={1}>
                        ...
                        <Box display="flex" alignItems="center" gap={0.5} mb={1} flexWrap="wrap">
                          ...
                          <Chip
                            ...
                            size="small"
                            color={budget.calculationType === 'debit' ? 'error' : 'info'}
                            variant="outlined"
                            sx={{ fontSize: '0.7rem' }}
                          />
                        </Box>
        ...
        <DialogContent sx={{ pt: 3 }}>
          <Box sx={{ mt: 2, display: 'flex', flexDirection: 'column', gap: 3 }}>
            ...
            <Box>
              ...
              >
                <ToggleButton value="debit">📉 Expenses Only</ToggleButton>
                <ToggleButton value="net">⚖️ Net (Expenses - Refunds)</ToggleButton>
              </ToggleButtonGroup>
              <Typography
                variant="caption"
              ...
              >
                ...
                {formData.calculationType === 'net' &&
                  'Track net expenses after refunds/returns (debit - credit)'}
              </Typography>
            </Box>
              ...
              <FormControlLabel
                control={
                  <Switch
                    ...
                    onChange={(e) => setFormData({ ...formData, enableRollover: e.target.checked })}
                    sx={{
      ...
      >
        ...
        <DialogContent sx={{ pt: 3 }}>
          <Typography variant="body2" color="text.secondary" mb={3}>
            Quick-start your budget with pre-configured templates. You can customize amounts and
            filters after selection.
          </Typography>
          <Grid container spacing={2}>
